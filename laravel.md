...
- app/Models
    - 로직에 필요한 모든 모델들을 포함하고 있습니다. Trait, Interface, 그리고 각 클래스와 관련 있는 여러 디자인 패턴의 클래스들도 포함합니다.
- app/Utils
...
#### 코드 삭제를 두려워하지 마세요.
- 사용하지 않는 레거시 코드(메소드, API 등을 모두 포함합니다.)는 항상 삭제해주세요. 삭제가 두려우면 통주석이나 `throw new Exception` 이라도 이용해주세요.
- 숨어있는 버그를 찾고, 예상치 못한 결과를 방어하기 위한 제일 효율적인 방법입니다. (또한, `Unit Tests` 를 돌려보는 것이 매우 큰 도움이 될 수 있습니다.)

#### 하드코딩과 땜빵을 하지마세요.
- 상황에 따라 다를 수는 있으나, 어쩔 수 없는 상황을 제외하고는 기본적으로 하드코딩과 땜빵을 최대한 지양하도록 합니다. 개발 일정이 살짝 지체 될 수 있다고 하더라도, 이는 추후 일어날 수 있는 예상치 못한 많은 문제들, 그리고 개선사항들에 대해 미리 대비하기 위해 매우 중요한 작업입니다. 당장의 개발일정을 앞당기기 위하여 처리한 것들이 나중에 부메랑이 되어 "구조적으로 할 수 없다", "리팩토링 해야한다", "프로젝트 첨부터 다시 개발해야한다" 등의 부메랑이 되어 돌아올 수 있습니다.
- 위와 같이 미래 대비를 위함도 있지만, 기본적으로 하드코딩과 땜빵을 습관적으로 하는 것은 '프로그래머'가 아닌 '코더'가 될 수 있는 지름길입니다.
- 하드코딩을 해야할 상황이 있다면 혼자서 고민하지말고 옆의 백엔드 개발자 동료와 상의/토론하여 최종 합의안을 도출하여 작업하도록 합니다.
...
- 둘의 차이는 http://stackoverflow.com/questions/5197300/new-self-vs-new-static 를 참조.
```
...
- 만약 타임존을 알 수 없고, 시간 값이 없이 날짜 값만 있으며, 그리고 특성상 단순 출력 용도의 느낌이 강한 데이터의 경우 `DATE` 타입을 활용하여 날짜 값만 저장하도록 하며 타임존은 `UTC`가 아닌 로컬 타임존으로 한다. 네이밍 컨벤션을 별도로 정해진바는 없으며 `date` 로 사용한 곳이 있다. 
